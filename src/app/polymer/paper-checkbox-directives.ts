/* Suggested Name File: paper-checkbox-directives.ts */
...
export class PaperCheckboxValidationDirective implements DoCheck {
    _element: any;
...
@Directive({
    selector: 'paper-checkbox',
    providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => PaperCheckboxFormElementDirective), multi: true }],
    host: { '(checkedChange)': 'onValueChanged($event)' }
})
export class PaperCheckboxFormElementDirective {
    _element: any;
    _renderer: any;
...
export class PaperCheckboxReloadConfigurationDirective {
