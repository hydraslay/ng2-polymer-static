/* Suggested Name File: vaadin-date-picker-directives.ts */
...
export class VaadinDatePickerValidationDirective implements DoCheck {
    _element: any;
...
@Directive({
    selector: 'vaadin-date-picker',
    providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => VaadinDatePickerFormElementDirective), multi: true }],
    host: { '(valueChange)': 'onValueChanged($event)' }
})
export class VaadinDatePickerFormElementDirective {
    _element: any;
    _renderer: any;
...
export class VaadinDatePickerNotifyForDiffersDirective implements OnInit, DoCheck {
    ...
    _differs: any;
    _arrayDiffs: any;
    arrayAndObjectProperties: any[] = ["keyEventTarget", "i18n"];
...
export class VaadinDatePickerReloadConfigurationDirective {
