/* Suggested Name File: paper-input-directives.ts */
...
export class PaperInputValidationDirective implements DoCheck {
    _element: any;
...
@Directive({
    selector: 'paper-input',
    providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => PaperInputFormElementDirective), multi: true }],
    host: { '(valueChange)': 'onValueChanged($event)' }
})
export class PaperInputFormElementDirective {
    _element: any;
    _renderer: any;
...
export class PaperInputReloadConfigurationDirective {
