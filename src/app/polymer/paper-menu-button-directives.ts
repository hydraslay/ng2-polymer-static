/* Suggested Name File: paper-menu-button-directives.ts */
...
export class PaperMenuButtonValidationDirective implements DoCheck {
    _element: any;
...
@Directive({
    selector: 'paper-menu-button',
    providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => PaperMenuButtonFormElementDirective), multi: true }],
    host: { '(valueChange)': 'onValueChanged($event)' }
})
export class PaperMenuButtonFormElementDirective {
    _element: any;
    _renderer: any;
...
export class PaperMenuButtonNotifyForDiffersDirective implements OnInit, DoCheck {
    ...
    _differs: any;
    _arrayDiffs: any;
    arrayAndObjectProperties: any[] = ["keyEventTarget", "openAnimationConfig", "closeAnimationConfig"];
...
export class PaperMenuButtonReloadConfigurationDirective {
